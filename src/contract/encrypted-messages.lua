local encrypted_messages = {}

function encrypted_messages.init()
	...
	EncryptionPublicKey = EncryptionPublicKey or ao.env.Process.Tags.EncryptionPublicKey
	...
	Handlers.add(
		...
		function(msg)
			assert(msg.From == ao.env.Process.Owner, "This action is only available to the process Owner")
	...
	Handlers.add(
		...
		function(msg)
			...
			assert(type(decoded.recipientPublicKey) == "string", "Invalid recipientPublicKey")
	...
	Handlers.add("getEncryptedMessage", Handlers.utils.hasMatchingTag("Action", "Get-Encrypted-Message"), function(msg)
