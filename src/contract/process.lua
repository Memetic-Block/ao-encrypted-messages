...
local encrypted_messages = require(".encrypted-messages")
...
function process.handle(msg, _)
	...
	encrypted_messages.init()

	-- to enable evolve capabilites renable this block
	--   Handlers.add("_eval",
	--     function (msg)
	...
	if not status then
		if msg.Action == "Eval" then
			error("Eval is not currently available")
